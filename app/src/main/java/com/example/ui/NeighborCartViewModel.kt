...
sealed interface UserLocation {
    ...
    object None : UserLocation {
        override val name = "Select Delivery Address"
    ...
    data class CustomAddress(override val name: String, override val latitude: Double, override val longitude: Double) : UserLocation
}
...
class NeighborCartViewModel(
            ...
            try {
                if (supabaseUrl.isNotEmpty() && supabaseKey.isNotEmpty()) {
                    ...
                    if (response.isNotEmpty()) {
                        currentLocation = UserLocation.CustomAddress(response.first().address, 0.0, 0.0)
                    }
                } else {
                    supabaseSavedAddresses = emptyList()
                }
    ...
    var currentLocation by mutableStateOf<UserLocation>(UserLocation.None)
        private set

    fun updateLocation(location: UserLocation) {
        currentLocation = location
        // Open delivery everywhere, always pick the first valid store
    ...
    val geofencedStoresFlow: StateFlow<List<GeofencedStore>> = combine(
        ...
        singleStoreList.map { store ->
            ...
            GeofencedStore(
                store = store,
                distanceKm = dist,
                isDeliverable = true // Open for everyone
            )
        }
